...
class JointDiBS(DiBS):
    ...
    def f_kernel(self, x_latent, x_theta, x_sigma, y_latent, y_theta, y_sigma, h_latent, h_theta, h_sigma, t):
        """
        Evaluates kernel
        ...
        return self.kernel.eval(
            x_latent=x_latent, x_theta=x_theta, x_sigma=x_sigma,
            y_latent=y_latent, y_theta=y_theta, y_sigma=y_sigma,
            h_latent=h_latent, h_theta=h_theta, h_sigma=h_sigma)
    

    def f_kernel_mat(self, x_latents, x_thetas, x_sigmas, y_latents, y_thetas, y_sigmas, h_latent, h_theta, h_sigma, t):
        """
        ...
        Returns:
            [A, B] kernel values
        """
        return vmap(vmap(self.f_kernel, (None, None, None, 0, 0, 0, None, None, None, None), 0), 
            (0, 0, 0, None, None, None, None, None, None, None), 0)(x_latents, x_thetas, x_sigmas, y_latents, y_thetas, y_sigmas, h_latent, h_theta, h_sigma, t)

    
    def eltwise_grad_kernel_z(self, x_latents, x_thetas, x_sigmas, y_latent, y_theta, y_sigma, h_latent, h_theta, h_sigma, t):
        """
        Computes gradient d/dz k((z, theta, sigma), (z', theta', sigma')) elementwise for each provided particle (z, theta)
        ...
        return vmap(grad_kernel_z, (0, 0, 0, None, None, None, None, None, None, None), 0)(x_latents, x_thetas, x_sigmas, y_latent, y_theta, y_sigma, h_latent, h_theta, h_sigma, t)


    def eltwise_grad_kernel_theta(self, x_latents, x_thetas, x_sigmas, y_latent, y_theta, y_sigma, h_latent, h_theta, h_sigma, t):
        """
        Computes gradient d/dtheta k((z, theta, sigma), (z', theta', sigma')) elementwise for each provided particle (z, theta)
        ...
        return vmap(grad_kernel_theta, (0, 0, 0, None, None, None, None, None, None, None), 0)(x_latents, x_thetas, x_sigmas, y_latent, y_theta, y_sigma, h_latent, h_theta, h_sigma, t)
    
    def eltwise_grad_kernel_sigma(self, x_latents, x_thetas, x_sigmas, y_latent, y_theta, y_sigma, h_latent, h_theta, h_sigma, t):
        """
        Computes gradient d/dtsigma k((z, theta, sigma), (z', theta', sigma')) elementwise for each provided particle (z, theta)
        ...
        Returns:
            batch of gradients for parameters (PyTree with leading dim `n_particles`)
        """
        grad_kernel_sigma = grad(self.f_kernel, 2)
        return vmap(grad_kernel_sigma, (0, 0, 0, None, None, None, None, None, None, None), 0)(x_latents, x_thetas, x_sigmas, y_latent, y_theta, y_sigma, h_latent, h_theta, h_sigma, t)


    def z_update(self, single_z, single_theta, single_sigma, kxx, z, theta, sigma, grad_log_prob_z, h_latent, h_theta, h_sigma, t):
        """
        ...
        repulsion = self.eltwise_grad_kernel_z(z, theta, sigma, single_z, single_theta, single_sigma, h_latent, h_theta, h_sigma, t)
    ...
    def parallel_update_z(self, *args):
        ...
        """
        return vmap(self.z_update, (0, 0, 0, 1, None, None, None, None, None, None, None, None), 0)(*args)


    def theta_update(self, single_z, single_theta, single_sigma, kxx, z, theta, sigma, grad_log_prob_theta, h_latent, h_theta, h_sigma, t):
        """
    ...
    def parallel_update_theta(self, *args):
        ...
        """
        return vmap(self.theta_update, (0, 0, 0, 1, None, None, None, None, None, None, None, None), 0)(*args)

    def sigma_update(self, single_z, single_theta, single_sigma, kxx, z, theta, sigma, grad_log_prob_sigma, h_latent, h_theta, h_sigma, t):
        """
        Computes SVGD update for `single_sigma` of a (single_z, single_theta, single_sigma) tuple given the kernel values 
        `kxx` and the d/dsigma gradients of the target density for each of the available particles 
    ...
    def parallel_update_sigma(self, *args):
        """
        Parallelizes `z_update` for all available particles
        Otherwise, same inputs as `z_update`.
        """
        return vmap(self.sigma_update, (0, 0, 0, 1, None, None, None, None, None, None, None, None), 0)(*args)

    # this is the crucial @jit
    @functools.partial(jit, static_argnums=(0,))
    def svgd_step(self, opt_state_z, opt_state_theta, opt_state_sigma, data, key, interv_targets, t, sf_baseline):
        """
        ...
        h_sigma = self.kernel.h_sigma

        # d/dtheta log p(theta, D | z, sigma)
        key, subk = random.split(key)
        dtheta_log_prob = self.eltwise_grad_theta_likelihood(z, theta, sigma, t, data, interv_targets, subk)

        # d/dz log p(theta, D | z, sigma)
        key, *batch_subk = random.split(key, n_particles + 1)
        dz_log_likelihood, sf_baseline = self.eltwise_grad_z_likelihood(z, theta, sigma, sf_baseline, t, data, interv_targets, jnp.array(batch_subk))
        ...
        # d/dsigma log p(theta, D | sigma, z)
        key, subk = random.split(key)
        dsigma_log_likelihood = self.eltwise_grad_sigma_likelihood(z, theta, sigma, t, data, interv_targets, subk)
        dsigma_log_prior = self.eltwise_grad_sigma_prior(sigma, t)

        dsigma_log_prob = dsigma_log_prior + dsigma_log_likelihood
        # k((z, theta), (z, theta)) for all particles
        kxx = self.f_kernel_mat(z, theta, sigma, z, theta, sigma, h_latent, h_theta, h_sigma, t)

        # transformation phi() applied in batch to each particle individually
        phi_z = self.parallel_update_z(z, theta, sigma, kxx, z, theta, sigma, dz_log_prob, h_latent, h_theta, h_sigma,t)
        phi_theta = self.parallel_update_theta(z, theta, sigma, kxx, z, theta, sigma, dtheta_log_prob, h_latent, h_theta, h_sigma, t)
        phi_sigma = self.parallel_update_sigma(z, theta, sigma, kxx, z, theta, sigma, dsigma_log_prob, h_latent, h_theta, h_sigma,t)
        ...
        opt_state_sigma = self.opt_update(t, phi_sigma, opt_state_sigma)

        return opt_state_z, opt_state_theta, opt_state_sigma, key, sf_baseline
    
    

    def sample_particles(self, *, n_steps, init_particles_z, init_particles_theta, init_particles_sigma, data, key, interv_targets, callback=None, callback_every=0):
        """
        ...
        opt_state_sigma = opt_init(sigma)
        ...
        sigma_final = self.get_params(opt_state_sigma)
        return z_final, theta_final, sigma_final
